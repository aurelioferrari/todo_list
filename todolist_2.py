...
layout = [
    ...
    [sg.T('Escreva a tarefa'), sg.I(key='task', font=('None 15'), size=(32, 1))],
    [sg.Table(selected_row_colors='yellow on black',alternating_row_color='green', values='', headings=['Índice', 'Horário', 'Tarefa'], key='Tabela', size=(2000, 10), auto_size_columns=False, col_widths=[5, 9, 30], vertical_scroll_only=False, justification='c', font='None 13', enable_click_events=True)],
    [sg.B('Adicionar', button_color='green', bind_return_key=True), sg.B('Deletar', key='deletar', button_color='red'), sg.B('Deletar Tudo', key='deletar_tudo', button_color='red'), sg.B('Topo da lista', key='topo', button_color='green'), sg.Push(), sg.B('Organizar', key='organizar')],
    [sg.T('Selecione a ordem da tarefa'), sg.I(key='novo', size=(3, 1)), sg.B('Mudar', key='mudar'), sg.Push(), sg.Exit('Sair')]

]

counter = 1
tasks = []
window = sg.Window('iFazer v1.11', layout, finalize=True)
counter_save = []
...
while True:
    ...
    if event == 'info':
        sg.popup_ok('Novidades da Versão 1.11:\n'
                    '- Botão Deletar Tudo foi Adicionado.\n'
                    '- Botões Organizar e Sair foram reposicionados.\n'
                    '- As linhas têm cores alternadas\n'
                    '- Horários adicionados no menu.\n'
                    '\nINSTRUÇÕES\n'
